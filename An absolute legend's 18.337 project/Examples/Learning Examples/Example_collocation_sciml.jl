...
asdf2 = reshape([sin.(tsteps) sin.(tsteps)], (2, 300)) .+ 0.1randn(2, 300)
asdf = range(1f0, 20f0, length = 120)
asdf3 = reshape([sin.(asdf) sin.(asdf)], (2, 120)) .+ 10.5randn(2, 120)
du, u = collocate_data(asdf2, tsteps, EpanechnikovKernel())
du, u = collocate_data(asdf3, asdf, EpanechnikovKernel())
...
## Train using the collocate initial parameters
